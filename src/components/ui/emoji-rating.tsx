...
export function RatingInteraction({ onChange, className, showLabel = true }: RatingInteractionProps) {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-6", className)}>
      ...
      {showLabel && <div className="relative h-7 w-32">
        ...
        >
          <span className="text-sm font-medium" style={{ color: 'var(--foreground-subtle)', opacity: 0.5 }}>Select a mood</span>
        </div>

        {/* Rating labels with blur in/out */}
        {ratingData.map((item, i) => (
          <div
            key={i}
            className={cn(
